...
def cars_predict(df):
    ...
    cars_predict_rf(df, input_data)

def cars_predict_rf(df, input_data):
    st.write("RandomForestRegressor 모델을 활용하여 자동차의 연비(mpg)를 예측합니다.")
...
def continent_mpg(df):
    ...
    # width="stretch": Streamlit 페이지의 가로 폭(컨테이너 너비)에 자동으로 맞춘다.

    st.markdown("---")

    st.info(""" 💡 **분석 포인트** - 대륙별로 차량 특성이 다르며, 미국 차량은 비교적 연비가 낮고 일본 차량은 연비가 높은 편입니다.   """)
...
def hp_mpg(df):
    ...
    st.markdown("---")
    st.info(""" 💡 **분석 포인트** - 마력이 높거울수록 연비가 낮아지는 경향이 있습니다.  """)
...
def weightlbs_mpg(df):
    ...
    st.info(""" 💡 **분석 포인트** - 차량 무게가 무거울수록 연비가 낮아지는 경향이 있습니다. """)
...
def year_mpg(df):
    ...
    st.info("""💡 **분석 포인트** - 연도별 평균 연비가 점차 개선되는 추세를 확인할 수 있습니다. """)

def cylinders_mpg(df):
    st.subheader("🔧 실린더 수별 연비 분포")

    plot_df = df.copy()
    plot_df["cylinders"] = plot_df["cylinders"].astype(str)
    ...
    st.info("""
    💡 **분석 포인트** - 실린더 수가 많을수록 연비가 낮아지는 경향이 있으며,
    같은 실린더 수에서도 제조 대륙에 따라 연비 분포 차이를 확인할 수 있습니다.
    """)
